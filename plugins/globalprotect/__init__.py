...
def testconnect(pluginargs, args, api_dict, useragent):
    ...
    resp = requests.get(api_dict['proxy_url'] + "/global-protect/login.esp", headers=headers, verify=False, proxies=pluginargs["proxy"])

    if resp.status_code == 504:
        output = "Testconnect: Connection failed, endpoint timed out, exiting"
        success = False
    elif "GlobalProtect Portal" in resp.text:
        output = "Testconnect: Verified paloalto GlobalProtect instance, connected"
    else:
        output = "Testconnect: Warning, paloalto GlobalProtect not indicated, continuing"
